#!/bin/sh
# install.sh — install officelens from GitHub.
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/officelens/main/install.sh | sh
#
# Installs the `officelens` binary globally with Bun. Pass a git ref as the
# first argument to pin a branch, tag, or commit:
#
#   curl -fsSL https://raw.githubusercontent.com/srivtx/officelens/main/install.sh | sh -s v0.1.0
set -eu

REPO="srivtx/officelens"
BIN="officelens"
REF="${1:-main}"
